...
// Describe an image (a previously generated frame) in ONE short sentence so a
// downstream node can be generated to continue it seamlessly. Used to honor
// node connections on the canvas (connect scenes / images for video gen).
async function describeFrame(dataUrl) {
  try {
    const { text } = await generateText({
      model: AI_MODEL,
      system:
        'You are a continuity supervisor. Describe this single film frame in ONE concise sentence (subject, setting, wardrobe, colors, camera angle) so the NEXT shot can continue it seamlessly. No preamble.',
      messages: [
        {
          role: 'user',
          content: [
            { type: 'text', text: 'Describe this frame for shot continuity:' },
...
// Build a continuity instruction from the upstream nodes connected to the one
// being generated. Only the last up-to-3 incoming frames are considered so the
// prompt stays focused. Returns a string, or null when there's nothing usable.
async function buildContinuity(refs) {
  if (!Array.isArray(refs) || refs.length === 0) return null;
  const cues = [];
  for (const r of refs.slice(-3)) {
    if (r?.poster) {
      const desc = await describeFrame(r.poster);
      if (desc) cues.push(desc);
      else if (r.prompt) cues.push(r.prompt);
    } else if (r?.prompt) {
      cues.push(r.prompt);
    }
  }
...
export async function generateNode({ prompt, model, mode, ratio, duration, refs }) {
  const clean = (prompt || '').trim() || 'an abstract cinematic moment';
  const continuity = await buildContinuity(refs);
  const framing = mode === '首尾帧'
    ? 'First/last frame anchored composition. '
    : 'Cinematic single key frame. ';
  let imagePrompt = `${framing}${clean}. Cinematic lighting, film still, high detail, 35mm look.`;
  let captionSystem = `${AGENT_SYSTEM}\nTask: in ONE sentence, describe the key frame this video prompt would produce. Vivid, visual, present tense.`;
  if (continuity) {
    const cont = continuity.join(' ');
    imagePrompt += ` Maintain continuity with the previous shot: ${cont}. Keep the same subject, wardrobe, setting and visual style.`;
    captionSystem += `\nThe shot continues directly from: ${cont}. Reflect that visual continuity in one sentence.`;
  }
  ...
  if (poster) {
    return { caption, poster, model, ratio, duration, source: 'ai', refsUsed: continuity?.length || 0 };
  }
  ...
  return {
    ...
    refsUsed: continuity?.length || 0,
  };
}

// Built-in style presets for the image-generation node.
const IMAGE_STYLES = {
  none: '',
  cinematic: 'cinematic lighting, film still, 35mm, shallow depth of field, high detail',
  anime: 'anime illustration, clean line art, soft cel shading, vibrant colors',
  '3d': 'stylised 3D render, soft global illumination, subsurface scattering, Pixar-like',
  photoreal: 'photorealistic, natural light, ultra-detailed, 8k, sharp focus',
  watercolor: 'watercolor painting, soft washes, paper texture, hand-painted',
  cyberpunk: 'cyberpunk, neon lights, rain-slick streets, moody atmosphere, high contrast',
};
export const IMAGE_STYLE_KEYS = Object.keys(IMAGE_STYLES);

// Generate a single IMAGE node's result — a real text-to-image render through
// the Gateway. Unlike generateNode (video key-frame) this needs no caption and
// applies an optional style preset. Falls back to a placeholder poster.
export async function generateImageNode({ prompt, model, ratio, style, refs }) {
  const clean = (prompt || '').trim() || 'an abstract artwork';
  const continuity = await buildContinuity(refs);
  const suffix = IMAGE_STYLES[style] || IMAGE_STYLES.cinematic;
  let imagePrompt = suffix ? `${clean}. ${suffix}.` : clean;
  if (continuity) {
    imagePrompt += ` Continue the visual style and subject from the reference image: ${continuity.join(' ')}. Match colors, lighting and composition.`;
  }

  const poster = await generatePoster(imagePrompt, ratio);
  if (poster) {
    return { poster, model, ratio, style, source: 'ai', refsUsed: continuity?.length || 0 };
  }
  const seed = encodeURIComponent(clean.slice(0, 60));
  const { w, h } = ratioToSize(ratio);
  return {
