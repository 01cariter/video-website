import 'server-only';
import { sql } from './db';
import { asCanvasJson, normalizeCanvas } from './canvas-state';

// ============================================================================
// Canvas persistence (Supabase Postgres).
//
...
export async function getOrCreateCanvas(userId, { name } = {}) {
  const existing = await sql`
    ...
    LIMIT 1
  `;
  if (existing.length) return normalizeCanvas(existing[0]);
  ...
  `;
  return normalizeCanvas(created);
}
...
export async function getCanvas(userId, projectId) {
  const [row] = await sql`
    ...
    LIMIT 1
  `;
  return normalizeCanvas(row || null);
}
...
export async function saveCanvas(userId, projectId, { nodes, edges, name }) {
  const [row] = await sql`
    UPDATE projects
    SET nodes = ${asCanvasJson(sql, nodes)},
        edges = ${asCanvasJson(sql, edges)},
